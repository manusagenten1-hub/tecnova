...
const Hero: React.FC = () => {
  ...
  return (
    <section ref={containerRef} id="hero" className="relative min-h-screen flex items-center pt-24 pb-12 bg-brand-dark overflow-hidden">
      ...
      {/* Background Gradients/Glows with Parallax & Hatchet Vibe */}
      <div className="absolute inset-0 z-0 bg-brand-dark overflow-hidden pointer-events-none">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(230,0,0,0.05)_0%,transparent_100%)]"></div>
      </div>
        ...
        >
          ...
          <div className="w-full md:w-3/5 text-left pr-0 md:pr-12">
            <h1 className="flex flex-wrap justify-start items-center gap-x-2 sm:gap-x-3 mb-4 md:mb-6">
              {["Estruturas", "Digitais", "Projetadas", "para"].map((word, i) => (
                <span key={i} className="overflow-hidden inline-flex pb-1 sm:pb-3">
                  <motion.span variants={wordVariants} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-heading font-bold text-white leading-[1.1] drop-shadow-2xl origin-bottom-left">
                    {word}
                  </motion.span>
                </span>
              ))}
              <span className="overflow-hidden flex flex-wrap justify-start pb-1 sm:pb-3 w-full mt-2 sm:mt-4 text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-heading font-black leading-[1.2] text-brand-accent drop-shadow-[0_0_25px_rgba(230,0,0,0.5)]">
            ...
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 justify-start mb-10 md:mb-12 w-full sm:w-auto mt-2 md:mt-6">
              <Button variant="primary" icon="arrow" onClick={() => window.dispatchEvent(new CustomEvent('open-quiz-modal'))} className="w-full sm:w-auto text-base md:text-lg px-8 py-4 md:px-10 md:py-5 font-bold uppercase tracking-wide bg-brand-accent hover:bg-brand-accentDark text-white shadow-[0_0_20px_rgba(230,0,0,0.4)] hover:shadow-[0_0_30px_rgba(230,0,0,0.6)]">
                Começar Meu Projeto
              </Button>
            </motion.div>
          ...
          <div className="w-full md:w-2/5 hidden md:flex items-center justify-center pl-4 lg:pl-12 relative z-10">
            <motion.img 
              ...
              alt="TecNova" 
              className="w-full max-w-[350px] lg:max-w-[450px] h-auto object-contain drop-shadow-[0_0_30px_rgba(255,255,255,0.1)]" 
            />
          </div>
